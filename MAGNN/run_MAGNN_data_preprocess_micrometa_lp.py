...
# changed microd to micrometa
micrometa_train, micrometa_val, micrometa_test = split_date(micrometa, train_ratio=0.7, val_ratio=0.2, test_ratio=0.1)
save_split_data2npz(micrometa_train, micrometa_val, micrometa_test, "data/sampled/preprocessed/micro_meta_train_val_test_idx.npz")

# training: 70%, validation: 20%, testing: 10%
train_val_test_idx = np.load("data/sampled/preprocessed/micro_meta_train_val_test_idx.npz")
...
microbe_metabolite = micrometa.loc[train_idx].reset_index(drop=True)
microbe_metabolite.head()
print(f"Length of Training data: {len(microbe_metabolite)}")
...
# write all things
target_idx_lists = [np.arange(num_microbe), np.arange(num_metabolite)]
offset_list = [0, num_microbe+num_disease]
